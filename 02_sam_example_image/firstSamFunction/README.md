...
```bash
firstSamFunction$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
firstSamFunction$ sam local start-api
firstSamFunction$ curl http://localhost:3000/
```
...
```bash
firstSamFunction$ sam logs -n HelloWorldFunction --stack-name firstSamFunction --tail
```
...
```bash
firstSamFunction$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name firstSamFunction
```
